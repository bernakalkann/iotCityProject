...
1. Python ortamınızda `paho-mqtt` kütüphanesini kurun:
   ```bash
   pip install paho-mqtt
   ```
2. `certs/` klasörüne AWS'den indirdiğiniz sertifikaları koyun.
3. Simülatörü çalıştırın:
   ```bash
   python sensor_sim.py
   ```
4. AWS Lambda konsolundaki **Monitor** -> **CloudWatch logs** sekmesinden veya AWS IoT Core üzerindeki **MQTT test client** aracından verilerin ulaştığını ve 35 derecenin üzerindeki sıcaklıklarda alarm tetiklendiğini gözlemleyebilirsiniz.
